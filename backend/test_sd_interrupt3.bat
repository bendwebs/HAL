@echo off
echo Testing with interrupt + 3 second delay between each
echo =====================================================
...
echo.
echo Generation 7...
curl -s -X POST http://127.0.0.1:7860/sdapi/v1/txt2img -H "Content-Type: application/json" -d "{\"prompt\":\"test\",\"steps\":5,\"width\":64,\"height\":64}" -o out7.json --max-time 30
if exist out7.json (echo SUCCESS) else (echo FAILED)
curl -s -X POST http://127.0.0.1:7860/sdapi/v1/interrupt >nul 2>&1
timeout /t 3 /nobreak >nul

echo.
echo Generation 8...
curl -s -X POST http://127.0.0.1:7860/sdapi/v1/txt2img -H "Content-Type: application/json" -d "{\"prompt\":\"test\",\"steps\":5,\"width\":64,\"height\":64}" -o out8.json --max-time 30
if exist out8.json (echo SUCCESS) else (echo FAILED)
